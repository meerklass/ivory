...
class TestLoop(ContextSensitiveTest):
    ...
    def test_one_plugin(self):
        mock_ctx = Struct(
            {"params": Struct({"SimplePlugin": Struct({"mock_parameter": 1})})}
        )
    ...
    def test_complex_loop(self):
        mock_ctx = Struct({"params": Struct({"SimplePlugin": Struct({"value": 1})})})
        loop = Loop(
            [PLUGIN_NAME, Loop([PLUGIN_NAME, PLUGIN_NAME], ctx=mock_ctx), PLUGIN_NAME],
            ctx=mock_ctx,
        )
    ...
    def test_loop_max_iter(self):
        ...
        loop = Loop(
            plugin_list, ctx=mock_ctx, stop=RangeStopCriteria(max_iter=max_iter)
        )
    ...
    def test_loop_max_iter_nested(self):
        mock_ctx = Struct(
            {"params": Struct({"SimplePlugin": Struct({"value": 1})}), "value": None}
        )
        max_iter = 3
        plugin_list = [SimplePlugin(mock_ctx), SimplePlugin(mock_ctx)]

        loop = Loop(
            Loop(plugin_list, stop=RangeStopCriteria(max_iter=max_iter), ctx=mock_ctx),
