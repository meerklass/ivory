...
class TestLoop(ContextSensitiveTest):
    ...
    def test_complex_loop(self):
        mock_ctx = Struct({"params": Struct({"SimplePlugin": Struct({"value": 1})})})
        loop = Loop([PLUGIN_NAME, Loop([PLUGIN_NAME, PLUGIN_NAME], ctx=mock_ctx), PLUGIN_NAME], ctx=mock_ctx)
    ...
    def test_loop_max_iter_nested(self):
        mock_ctx = Struct({"params": Struct({"SimplePlugin": Struct({"value": 1})}), "value": None})
        max_iter = 3
